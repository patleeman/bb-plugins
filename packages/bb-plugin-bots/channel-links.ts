const channelPath =
  /^\/plugins\/bots\/channels\/([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})(?:\/message\/([^/]+))?\/?$/iu;
...
export function channelMessageReference(
  ...
  const label = `Message in #${name}`
    .replace(/[\\[\]<>*_`]/gu, "\\$&")
    .replace(/[\r\n]/gu, " ");
  return `[${label}](/plugins/bots/channels/${encodeURIComponent(roomId)}/message/${encodeURIComponent(messageId)})`;
}
...
export function channelLinkDestination(
  ...
  try {
    ...
    return `${id}/message/${encodeURIComponent(messageId)}`;
  } catch {
    return null;
  }
}
